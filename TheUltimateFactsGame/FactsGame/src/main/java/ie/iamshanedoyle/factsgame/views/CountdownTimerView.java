...
/**
 * Created by shane on 16/01/2014.
 */
public class CountdownTimerView extends View {
    ...
    public CountdownTimerView(Context context, AttributeSet attrs) {
        ...
        parseAttributes(context.obtainStyledAttributes(attrs,
                R.styleable.CountdownTimerView));
    }
    ...
    private void parseAttributes(TypedArray a) {
        barWidth = (int) a.getDimension(R.styleable.CountdownTimerView_barWidth,
                barWidth);

        rimWidth = (int) a.getDimension(R.styleable.CountdownTimerView_rimWidth,
                rimWidth);

        delayMillis = a.getInteger(R.styleable.CountdownTimerView_delayMillis,
                delayMillis);
        if (delayMillis < 0) {
            delayMillis = 0;
        }

        barColor = a.getColor(R.styleable.CountdownTimerView_barColor, barColor);

        barLength = (int) a.getDimension(R.styleable.CountdownTimerView_barLength,
                barLength);

        textSize = (int) a.getDimension(R.styleable.CountdownTimerView_textSize,
                textSize);

        textColor = (int) a.getColor(R.styleable.CountdownTimerView_textColor,
                textColor);

        //if the text is empty , so ignore it
        if (a.hasValue(R.styleable.CountdownTimerView_text)) {
            setText(a.getString(R.styleable.CountdownTimerView_text));
        }

        rimColor = (int) a.getColor(R.styleable.CountdownTimerView_rimColor,
                rimColor);

        circleColor = (int) a.getColor(R.styleable.CountdownTimerView_circleColor,
                circleColor);
    ...
    public void incrementProgress(String textValue) {
        setText(textValue);
        isSpinning = false;
        progress+= 3.63;
        if (progress > 360) {
